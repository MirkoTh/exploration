...
function setup_experiment() {
    ...
    for (let idx = 0; idx < experiment_info["n_trials"]; idx++) {
        ...
        trial_info["vals_bandit_1"][idx] = Array(trial_info["n_vals_prep"]).fill().map(
            () => Math.round(randn_bm() * experiment_info["bandit_sd"] + experiment_info["bandit_means"][0])
        );
        trial_info["vals_bandit_2"][idx] = Array(trial_info["n_vals_prep"]).fill().map(
            () => Math.round(randn_bm() * experiment_info["bandit_sd"] + experiment_info["bandit_means"][1])
        );
    }
    console.log("first vals for bandit 1 are: " + trial_info["vals_bandit_1"][0])
    console.log("first vals for bandit 2 are: " + trial_info["vals_bandit_2"][0])
    console.log("last vals for bandit 1 are: " + trial_info["vals_bandit_1"][63])
    console.log("last vals for bandit 2 are: " + trial_info["vals_bandit_2"][63])
